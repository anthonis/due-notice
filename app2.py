...
input_buffer = st.file_uploader("Upload .csv file", type=("dat")) 
#file_selector(folder_path='.')
#print(file_selector)


#b = BytesIO(input_buffer.read().base64.b64encode('utf8'))
#file_selector(input_buffer)
...
if input_buffer is not None:
	#with open(input_buffer,'w+') as f:
	#	print(input_buffer.getvalue(),file=f)
	print('file uploaded')
	print(type(input_buffer))
	byte_str = input_buffer.read()
	print(type(byte_str))
	s=str(byte_str ,'unicode_escape')

	data11 = StringIO(s) 

	df=pd.read_csv(data11, error_bad_lines=False)


	text_obj = byte_str.decode('unicode_escape')
	print(type(text_obj))
	#print(input_buffer.path())
	#csvfile = TextIOWrapper(input_buffer, encoding=request.encoding)
	#print(type(csvfile))
	#file_spec = list(set(dir(_io.TextIOWrapper)).union(set(dir(input_buffer))))
	#print(dir(input_buffer))
	#print(file_spec)
	print('read')
	#input_buffer = list(set(dir(_io.TextIOWrapper)).union(set(dir(_io.BytesIO))))
	#file = get_file_spec()
	#print(type(file))
